...
| [test_provider_protocol.py](../tests/test_provider_protocol.py) | - | GPT-OSS model normalization, profile reasoning options, provider-neutral behavior, HTTP error classification, and response-status helpers. |
| [test_ai_response_recovery.py](../tests/test_ai_response_recovery.py) | - | Intentional idle/parse/schema/unsupported distinctions, required command fields, direct-user-only one-shot repair, streaming publication boundaries, and exactly-once history/memory behavior. |
