...
def _refresh_cache_unlocked() -> None:
    """Refresh in-memory entries from disk on mtime/size miss. Caller must hold `_lock`."""
    ...
    try:
        ...
        logger.warning(f"memory_search: stat failed: {exc}")
        return

    if (
        _cache_mtime is not None
        and mtime == _cache_mtime
        and size == _cache_size
    ):
        return
...
def get_longterm_entry_count() -> int:
    """Return cached entry count without copying the full entry list."""
    with _lock:
        _refresh_cache_unlocked()
...
def format_session_recap_for_prompt(
    *,
    episodic_limit: int = 5,
    longterm_limit: int = 3,
) -> str:
    """Read-only launch context from recent episodic + newest longterm entries.

    Safety: untrusted context only — never instructions to mutate the OS.
    """
    try:
        episodic_limit = max(0, min(int(episodic_limit), 20))
    except (TypeError, ValueError):
        episodic_limit = 5
    try:
        longterm_limit = max(0, min(int(longterm_limit), 10))
    except (TypeError, ValueError):
        longterm_limit = 3
    ...
    try:
        ...
        for entry in get_recent_memories(episodic_limit):
            summary = str(entry.get("summary", "") or "").strip()
            if summary:
                lines.append(f"- (episodic) {summary[:200]}")
    except Exception as exc:
        logger.debug(f"session_recap: episodic skipped: {exc}")

    try:
        entries = _load_entries()
        if longterm_limit > 0 and entries:
            for entry in entries[-longterm_limit:]:
                summary = str(entry.get("summary", "") or "").strip()
                if not summary:
                    continue
                ts = entry.get("ts", "")
                prefix = f"- (longterm {ts}) " if ts else "- (longterm) "
                lines.append(prefix + summary[:200])
    except Exception as exc:
        logger.debug(f"session_recap: longterm skipped: {exc}")
